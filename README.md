# iFont — インクルーシブ字幕研究用フォント刺激ジェネレータ

「字幕の表示パラメータ p% ≈ 視覚的認識確率 p%」を目指す研究の刺激素材と、
視覚版 Kikiwake 実験用のセルフパイロット / 本実験 (jsPsych) ページ。

**設計判断の根拠・経緯・残作業 (倫理審査含む TODO) は Cosense にまとめている**:
👉 **https://scrapbox.io/qurihara/iFont**

引き継ぎや新マシンでの作業継続の入り口も Cosense ページ末尾の
「リポジトリ構成」「設計判断のまとめ」「新マシンでの Claude Code セッション開始時の入り口プロンプト案」を参照。
